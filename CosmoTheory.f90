...
!------
! MagCosmoMC: adding new Cls...
        Type(TSkyPowerSpectrum), allocatable :: Cls(:,:), MagPassCls(:,:), MagVecCompCls(:,:), MagScalCompClsDD(:,:), MagScalCompClsPP(:,:), MagScalCompClsDP(:,:)
!------
...
!----
! MagCosmoMC: Passive Cls
    allocate(this%MagPassCls(Settings%num_cls,Settings%num_cls))
    do i=1,Settings%num_cls
        do j= i, 1, -1
            if (Settings%cl_lmax(i,j) >0) &
            & allocate(this%MagPassCls(i,j)%Cl(Settings%cl_lmax(i,j)), source=0._mcp)
        end do
    end do
!-----

!----
! MagCosmoMC: vector compensated cls
    allocate(this%MagVecCompCls(Settings%num_cls,Settings%num_cls))
    do i=1,Settings%num_cls
        do j= i, 1, -1
            if (Settings%cl_lmax(i,j) >0) &
            & allocate(this%MagVecCompCls(i,j)%Cl(Settings%cl_lmax(i,j)), source=0._mcp)
        end do
    end do
!-----

!----
! MagCosmoMC: scalar compensated
    allocate(this%MagScalCompClsDD(Settings%num_cls,Settings%num_cls))
    do i=1,Settings%num_cls
        do j= i, 1, -1
            if (Settings%cl_lmax(i,j) >0) &
            & allocate(this%MagScalCompClsDD(i,j)%Cl(Settings%cl_lmax(i,j)), source=0._mcp)
        end do
    end do
!-----
...
!----
! MagCosmoMC: scalar compensated
    allocate(this%MagScalCompClsPP(Settings%num_cls,Settings%num_cls))
    do i=1,Settings%num_cls
        do j= i, 1, -1
            if (Settings%cl_lmax(i,j) >0) &
            & allocate(this%MagScalCompClsPP(i,j)%Cl(Settings%cl_lmax(i,j)), source=0._mcp)
        end do
    end do
!-----
...
!----
! MagCosmoMC: scalar compensated
    allocate(this%MagScalCompClsDP(Settings%num_cls,Settings%num_cls))
    do i=1,Settings%num_cls
        do j= i, 1, -1
            if (Settings%cl_lmax(i,j) >0) &
            & allocate(this%MagScalCompClsDP(i,j)%Cl(Settings%cl_lmax(i,j)), source=0._mcp)
        end do
    end do
!-----
